...
const Inbox: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `company_id=eq.${companyId}` }, async (payload) => {
        const newMessage = payload.new;
        
        // Update specific conversation messages if selected
        if (newMessage.conversation_id === selectedConvId) {
          ...
          setConversations(prev => prev.map(c => 
            c.id === newMessage.conversation_id 
              ? { ...c, messages: [...c.messages, formatted], lastMessage: newMessage.content, timestamp: 'Just now' } 
              : c
          ));
        } else {
          ...
          setConversations(prev => prev.map(c => 
            c.id === newMessage.conversation_id 
              ? { ...c, lastMessage: newMessage.content, timestamp: 'Just now', isUnread: true } 
              : c
          ));
        }
      })
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      alert("Message failed to sync with system.");
    }
  };
    ...
    <div className="h-full flex animate-in fade-in duration-500 overflow-hidden text-slate-900 bg-white">
      ...
      <div className="w-64 border-r border-slate-100 flex flex-col p-4 bg-white z-30">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide space-y-2">
          ...
          <button 
            onClick={() => setActiveSection('internal-chat')}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all font-black text-[11px] uppercase tracking-widest ${
        ...
        <div className="p-5 border-b border-slate-50">
          <div className="relative mb-5">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-2xl text-[11px] font-bold outline-none focus:ring-2 focus:ring-blue-500/20 transition-all text-slate-900"
            />
          </div>
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] truncate font-bold text-slate-400 leading-none mb-2">
                      {conv.lastMessage}
                    </p>
                    <div className={`p-1 w-fit rounded ${getSourceColor(conv.source)}`}>
          ...
          <>
            <div className="px-6 py-5 border-b border-slate-50 flex items-center justify-between z-10 bg-white/80 backdrop-blur-md">
              ...
              <div className="flex items-center gap-2">
                <button className="p-2.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all"><Tag size={18} /></button>
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50/40 p-10 space-y-8 flex flex-col scrollbar-hide">
              {selectedConv.messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[85%] ${isMe ? 'self-end' : 'self-start'} animate-in slide-in-from-bottom-1`}>
            ...
            <div className="border-t border-slate-100 p-6 bg-white shadow-2xl z-20">
              ...
              <div className="relative border border-slate-200 rounded-[2.5rem] p-5 bg-slate-50/30 focus-within:bg-white transition-all shadow-inner">
                <textarea 
                  ...
                  className="w-full bg-transparent border-none outline-none text-sm font-bold text-slate-800 resize-none h-24"
                />
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                  ...
                  <button onClick={handleSendMessage} className="px-10 py-3 bg-blue-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-blue-600/30 hover:bg-blue-700 transition-all active:scale-95 flex items-center gap-3">
