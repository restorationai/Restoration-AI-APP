...
import { 
  ...
  MapPin, 
  X,
  // Fix: Add missing Save icon import
  Save,
  CheckCircle2,
  CalendarDays,
  Activity,
...
import { fetchCalendarEvents, syncCalendarEventToSupabase, fetchTechniciansFromSupabase, fetchCompanySettings, syncContactToSupabase, fetchContactsFromSupabase, fetchJobsFromSupabase, getCurrentUser } from '../lib/supabase';
...
const UnifiedCalendar: React.FC = () => {
  ...
  const handleCreateJob = async (e: React.FormEvent) => {
    ...
    // Constraint: One Open Job per Primary Contact
    const openJob = jobs.find(j => j.contactId === newJob.contactId && j.status === 'Open');
    if (openJob) {
      alert(`Client has an open file: "${openJob.title}". Close it to open a new job.`);
      return;
    }
    ...
    try {
      ...
      if (newJob.id) setEvents(prev => prev.map(ev => ev.id === newJob.id ? updatedEvent : ev));
      else setEvents(prev => [...prev, updatedEvent]);
      setIsBooking(false);
      setNewJob({id: null, title: '', type: 'inspection', contactId: '', jobId: '', location: '', city: '', zip: '', date: new Date(), time: `${new Date().getHours().toString().padStart(2, '0')}:00`, lossType: '', customLossType: '', techIds: []});
      setSelectedDate(d);
    } catch (err: any) {
  ...
  const filteredContactsList = useMemo(() => {
    // Exclude Property Managers as primary owners per logic request
